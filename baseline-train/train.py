    ...
    for ts in tqdm(range(hp.episode_len), desc=f'Worker {i}'):
        ...
        for k,(state,blocked) in states.items():
            ...
            if blocked:
                ...
                out = agents[i].get_action((state,blocked))
                if agents[i].training:
                    action,value,prob,pi_mcts = out
                    memories[i] = (state,action,value,prob,pi_mcts)
                    actions[k] = action
                else:
                    actions[k] = out
...
if __name__ == '__main__':
    ...
    ap.add_argument('--use-mcts', action='store_true', help='Enable MuZero-style planner for policy improvement')
    ap.add_argument('--num-sim', type=int, default=16, help='Number of MCTS simulations (if enabled)')
    ap.add_argument('--c-puct', type=float, default=1.5, help='PUCT exploration constant')
    ap.add_argument('--temperature', type=float, default=1.0, help='Visit-count temperature for pi (train)')
    ap.add_argument('--temperature-eval', type=float, default=0.1, help='Visit-count temperature for pi (eval)')
    ap.add_argument('--dirichlet-eps', type=float, default=0.25, help='Dirichlet noise epsilon at root (train)')
    ap.add_argument('--dirichlet-alpha', type=float, default=0.3, help='Dirichlet noise alpha at root (train)')
    ap.add_argument('--no-dynamic-cpuct', action='store_true', help='Disable dynamic c_puct schedule (use fixed)')
    ap.add_argument('--lambda-pi', type=float, default=0.5, help='Weight for MCTS distillation loss')
    ap.add_argument('--mcts-head-loss', type=float, default=0.0, help='Weight for auxiliary MCTS head losses (disabled by default)')
    ap.add_argument('--workers', type=int, default=None, help='Override: how many envs can run in parallel')
    ap.add_argument('--episode-len', type=int, default=None, help='Override: episode length')

    args = ap.parse_args()
    print(args)

    # Allow runtime overrides for resource control
    if args.workers is not None:
        HYPER_PARAMS.workers = args.workers
    # Force eval-aligned episode length to avoid mismatch
    if args.episode_len is not None and args.episode_len != 500:
        print(f"[WARN] Overriding episode_len={args.episode_len} to 500 to match eval.")
    ...
    agents = [InductiveGraphPPOAgent(
        ...
        c_puct=args.c_puct,
        temperature=args.temperature,  # backward compat
        temperature_train=args.temperature,
        temperature_eval=args.temperature_eval,
        dirichlet_epsilon=args.dirichlet_eps,
        dirichlet_alpha=args.dirichlet_alpha,
        use_dynamic_c_puct=not args.no_dynamic_cpuct,
        lambda_pi=args.lambda_pi,
        mcts_head_loss_w=args.mcts_head_loss
